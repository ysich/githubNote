...
| 增量构建 | 支持 | SBP对于增量构建的逻辑封装为 \`[BuildCache](https://zhida.zhihu.com/search?content_id=170188445&content_type=Article&match_order=1&q=BuildCache&zhida_source=entity)\` 类实现。 |
...
| AssetBundle Manifest | 行为变更 | SBP 提供了 \`\`[CompatibilityAssetBundleManifest](https://zhida.zhihu.com/search?content_id=170188445&content_type=Article&match_order=1&q=CompatibilityAssetBundleManifest&zhida_source=entity)\`\` 来适配 原来的 Manifest，并且新增了获取 Bundle CRC的接口 |
