# 关于Unity内存管理资源卸载加载之最详细解说没有之一 - 知乎
来自清华大学懒猫的分享，
------------

此猫早已不在触及Unity的工作，在日本Line工作多年，这他早年文章（**2012年**），很值得看

但现在（**2020**）成熟的团队如果还没把下面的内容封装好，那就太呕了，当然80%的团队还真没搞好AssetBundle加载和卸载，

有什么样的人就有什么样的集体，什么样的程序员就出什么样的程序

80%的团队没搞好，是因为80%的人。。。。。

本来有多场景处理，加上各个团队都会做自己的场景管理，内存分配和释放这块本没什么大问题（问题关注点，转移为场景间的过渡和处理）

起码多了一个内存释放和管理的触发点。。。巴适的很

*   ？？测试颜色标签？？

**<color="red">但最近发现很多团队出现返祖现象，一整个游戏或AR APP整个生命周期仅采用单Scane + 动态加载 + 佛系释放</color>**

但是思想返祖，付出的体力和韧性就不如当年的人；结果就悲剧了

_这是大神致敬系列，大部分直接转载大神在2012年写的文章，最后付上“一张图”，如果通篇看完，相信最后的一张图会让你廓然开朗；如果已有一定基础，或者项目困惑中，可直接掠过文字，跳到最后“一张图”。_
...
Unity3D 里有两种动态加载机制：一个是[Resources.Load](https://zhida.zhihu.com/search?content_id=162797789&content_type=Article&match_order=1&q=Resources.Load&zhida_source=entity)，另外一个通过AssetBundle,其实两者区别不大。 Resources.Load就是从一个缺省打进程序包里的AssetBundle里加载资源，而一般AssetBundle文件需要你自己创建，运行时 动态加载，可以指定路径和来源的。
...
一个Prefab从assetBundle里Load出来 里面可能包括：
...
*   和各种其他Assets。  
    ...
    等到没有任何 游戏场景物体在用这些Assets以后，这些assets就成了没有引用的游离数据块了，是UnusedAssets了，这时候就可以通过 [Resources.UnloadUnusedAssets](https://zhida.zhihu.com/search?content_id=162797789&content_type=Article&match_order=1&q=Resources.UnloadUnusedAssets&zhida_source=entity)来释放,Destroy不能完成这个任 务，AssetBundle.Unload(false)也不行，AssetBundle.Unload(true)可以但不安全，除非你很清楚没有任何 对象在用这些Assets了。  
    配个图加深理解：

![](https://pica.zhimg.com/v2-d6c9303bc3bd150566af635476e5f920_1440w.jpg)
...
如果需要立即释放内存加上[GC.Collect](https://zhida.zhihu.com/search?content_id=162797789&content_type=Article&match_order=1&q=GC.Collect&zhida_source=entity)()，否则内存未必会立即被释放，有时候可能导致内存占用过多而引发异常。  
...
![](https://pic3.zhimg.com/v2-6a879200cb7f9a40ed631cfb08dce93c_1440w.jpg)
...
![](https://pic4.zhimg.com/v2-05bdd1f91bb2377969fc798ec4e01119_1440w.jpg)
...
![](https://pica.zhimg.com/v2-8e76b359dae915c20ae818606c080086_1440w.jpg)

参考资料：
-----

（CSDN真的有点吃相太难看，我转贴这个链接的时候，这个参考资料还不需要收费的，免费可以查阅的资料）

。。。。但作为一个平台考虑，创收也无可厚非，并非作者本意设置要付费阅读，而是CSDN系统判断作者多天没上线，也联系不上，所以打了个包，直接默认设置为包月会员付费内容，世界的运行规律就这样，）
